...
起源：文本压缩算法，后被 OpenAI 用于 GPT
策略：自下而上、贪心高频合并
文本拆为最小字符 / 字节单元
统计相邻子词出现频次
每次合并频次最高的相邻组合
迭代直到词表达标
特点：只看频次，无语义 / 概率建模，规则简单
代表：GPT、LLaMA、Qwen、Mistral
2. WordPiece（谷歌）
策略：类 BPE 合并，但不看纯频次，看语言似然概率
优先合并能提升句子整体概率的子词对
特点：偏向语义贴合，易拆分稀有词，偏向理解类任务
...
4. SentencePiece
定位：不是独立分词算法，是分词工具库
内置内核：仅支持 SP-BPE、SP-Unigram 两种
独有优势：
无需提前空格分词，直接处理无分隔语言（中日韩）
原生支持字节编码，跨语言零适配
用法：开发者自选底层用 BPE 还是 Unigram
二、四大算法直观区别表
表格
算法	合并逻辑	优化方式	训练速度	推理速度	OOV 能力	适用场景
BPE	高频相邻合并	局部贪心	极快	最快	极强 (字节级)	生成式大模型
WordPiece	概率优先合并	局部语义优先	中等	快	一般	分类、语义理解
Unigram	删减低权重子词	全局概率最优	很慢	中等	强	多语言精细分词
SentencePiece	封装 BPE/Unigram	看所选内核	依内核而定	依内核而定	极强	跨语言统一分词
三、为什么主流 LLM 全都选用 BPE（字节级 BPE）
1. 训练构建词表成本极低
海量互联网预训练语料下，频次统计贪心合并远比 Unigram 全局概率优化、WordPiece 概率计算高效，大规模训练首选。
2. 字节级 BPE 彻底消灭未登录词 OOV
现代 LLM 全部用UTF-8 字节 BPE，把所有文本转为字节单元：
生僻字、表情、代码、乱码、特殊符号全能拆分
不存在<UNK>未知 token，完美适配开放式对话、代码生成、杂文本输入
3. 自回归生成场景适配性最强
LLM 核心是逐字续写生成：
BPE 拆分均匀、序列连续性好，生成流畅度高
WordPiece 偏向语义理解，拆分偏细碎，不利于长文本连贯生成
4. 推理速度最优，适配大模型部署
BPE 分词逻辑固定查表，无概率计算、无全局搜索，批量推理吞吐量最高，是 vLLM、TensorRT 等推理框架最优适配分词。
5. 工程生态完全成熟
GPT 系列率先普及，全网预训练权重、微调框架、加速库全部原生兼容 BPE，改造成本为 0。
6. 对比劣势算法优势
弃 WordPiece：偏向理解，不适合生成，合并规则死板
弃 Unigram：训练慢、开销大，生成场景不需要全局最优分词
SentencePiece 只是工具壳：多数多语言 LLM 仅用它封装BPE 内核，本质还是 BPE
四、最终选型总结
生成式大模型（GPT/LLaMA/ 开源 LLM） → 字节级 BPE
语义理解、分类模型（BERT 系） → WordPiece
小众多语言精细分词 → Unigram
跨语言统一开发 → SentencePiece + 底层 BPE
